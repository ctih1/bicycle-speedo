...
lv_obj_t *create_settings_part(
    ...
    if(settings_type == CLICK) {
        ...
        lv_obj_add_event_cb(settings_element, callback, LV_EVENT_CLICKED, (void*)(intptr_t)action_id);
        ...
        lv_obj_add_event_cb(settings_element, callback, LV_EVENT_VALUE_CHANGED, (void*)(intptr_t)action_id);
...
lv_obj_t *create_settings_button(lv_obj_t *view, char label[64], char text[64], int x, int y, int action_id, void (*callback)(lv_event_t*)) {
    ...
    if (label == "") {
        lv_obj_t *desc_label = lv_label_create(view);
